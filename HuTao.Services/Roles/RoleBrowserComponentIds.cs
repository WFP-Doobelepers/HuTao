namespace HuTao.Services.Roles;

public static class RoleBrowserComponentIds
{
    public const string RoleSelectId = "roles:role";
    public const string BackButtonId = "roles:back";
    public const string SearchButtonId = "roles:search";
    public const string ClearButtonId = "roles:clear";
    public const string RefreshButtonId = "roles:refresh";

    public const string SearchModalId = "roles:search-modal";
    public const string SearchInputId = "roles:q";
}

