...
public class DiscordSocketListener
{
    ...
    private Task OnChannelCreatedAsync(SocketChannel channel)
        => PublishScopedAsync(new ChannelCreatedNotification(channel), _cancellationToken);

    private Task OnChannelUpdatedAsync(SocketChannel oldChannel, SocketChannel newChannel)
        => PublishScopedAsync(new ChannelUpdatedNotification(oldChannel, newChannel), _cancellationToken);

    private Task OnConnectedAsync()
        => PublishScopedAsync(ConnectedNotification.Default, _cancellationToken);

    private Task OnDisconnectedAsync(Exception arg)
        => PublishScopedAsync(new DisconnectedNotification(arg), _cancellationToken);

    private Task OnGuildAvailableAsync(SocketGuild guild)
        => PublishScopedAsync(new GuildAvailableNotification(guild), _cancellationToken);

    private Task OnGuildMemberUpdatedAsync(
        Cacheable<SocketGuildUser, ulong> oldMember, SocketGuildUser newMember)
        => PublishScopedAsync(new GuildMemberUpdatedNotification(oldMember, newMember), _cancellationToken);

    private Task OnInteractionCreatedAsync(SocketInteraction interaction)
        => PublishScopedAsync(new InteractionCreatedNotification(interaction), _cancellationToken);

    private Task OnJoinedGuildAsync(SocketGuild guild)
        => PublishScopedAsync(new JoinedGuildNotification(guild), _cancellationToken);

    private Task OnMessageDeletedAsync(
        Cacheable<IMessage, ulong> message, Cacheable<IMessageChannel, ulong> channel)
        => PublishScopedAsync(new MessageDeletedNotification(message, channel), _cancellationToken);

    private Task OnMessageReceivedAsync(SocketMessage message)
        => PublishScopedAsync(new MessageReceivedNotification(message), _cancellationToken);
    ...
    private Task OnMessagesBulkDeletedAsync(
        ...
        => PublishScopedAsync(new MessagesBulkDeletedNotification(messages, channel), _cancellationToken);
    ...
    private Task OnMessageUpdatedAsync(
        ...
        => PublishScopedAsync(new MessageUpdatedNotification(oldMessage, newMessage, channel), _cancellationToken);
    ...
    private Task OnReactionAddedAsync(
        ...
        => PublishScopedAsync(new ReactionAddedNotification(message, channel, reaction), _cancellationToken);

    private Task OnReactionRemovedAsync(
        Cacheable<IUserMessage, ulong> message, Cacheable<IMessageChannel, ulong> channel, SocketReaction reaction)
        => PublishScopedAsync(new ReactionRemovedNotification(message, channel, reaction), _cancellationToken);

    private Task OnReadyAsync()
        => PublishScopedAsync(ReadyNotification.Default, _cancellationToken);

    private Task OnRoleCreatedAsync(SocketRole role)
        => PublishScopedAsync(new RoleCreatedNotification(role), _cancellationToken);

    private Task OnRoleUpdatedAsync(SocketRole oldRole, SocketRole newRole)
        => PublishScopedAsync(new RoleUpdatedNotification(oldRole, newRole), _cancellationToken);

    private Task OnUserBannedAsync(SocketUser user, SocketGuild guild)
        => PublishScopedAsync(new UserBannedNotification(user, guild), _cancellationToken);

    private Task OnUserJoinedAsync(SocketGuildUser guildUser)
        => PublishScopedAsync(new UserJoinedNotification(guildUser), _cancellationToken);

    private Task OnUserLeftAsync(SocketGuild guild, SocketUser user)
        => PublishScopedAsync(new UserLeftNotification(guild, user), _cancellationToken);

    private Task OnUserVoiceStateUpdatedAsync(SocketUser user, SocketVoiceState old, SocketVoiceState @new)
        => PublishScopedAsync(new UserVoiceStateNotification(user, old, @new), _cancellationToken);
}