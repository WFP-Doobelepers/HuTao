...
public class ModerationInteractionHandlers : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task HandleHistoryExpandCollapseAsync(string userIdString)
    {
        ...
        // Toggle the appropriate expanded state based on current mode
        if (state.IsGrouped)
        {
            // In grouped mode: toggle full vs truncated reasons (no page count change)
            state.IsGroupedExpanded = !state.IsGroupedExpanded;
        }
        else
        {
            // In chronological mode: toggle 5 vs 10 items per page (page count changes)
