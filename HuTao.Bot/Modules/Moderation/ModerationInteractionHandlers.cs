...
public class ModerationInteractionHandlers : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task HandleTimeoutRemoveAsync(string timeoutIdString)
    {
        ...
        try
        {
            ...
            {
                await FollowupAsync("Invalid timeout ID.", ephemeral: true);
                return;
            }
            ...
            if (timeout is null)
            {
                await FollowupAsync("Timeout not found or already removed.", ephemeral: true);
                return;
            }
            ...
            {
                await FollowupAsync("Failed to remove timeout. The user may not be timed out.", ephemeral: true);
            }
        }
        catch (Exception ex)
        {
    ...
    public async Task HandleTimeoutDetailsAsync(string timeoutIdString)
    {
        ...
        try
        {
            ...
            if (timeout is null)
            {
                await FollowupAsync("Timeout not found.", ephemeral: true);
                return;
            }
            ...
            var embed = new EmbedBuilder()
                ...
                .AddField("Moderator", timeout.Action?.Moderator is { } mod ? mod.Id.ToString() : "System", true)
                .AddField("Date", timeout.Action?.Date.ToString("MMM dd, yyyy HH:mm") ?? "Unknown", true)
                .WithFooter($"Timeout ID: {timeout.Id}")
                .WithTimestamp(DateTimeOffset.UtcNow);

            if (timeout.Category is not null)
