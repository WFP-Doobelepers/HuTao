...
public class InteractiveModerationModule : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task BanAsync(
        ...
        [Autocomplete(typeof(MessageAutocomplete<IUserMessage>))] IUserMessage? context = null,
        ...
        [RequireEphemeralScope] bool ephemeral = false)
    {
        ...
        var details = await GetDetailsAsync(user, reason, context, category, ephemeral);
    ...
    {
        await DeferAsync(ephemeral);
        var details = await GetDetailsAsync(user, reason, message, category, ephemeral);
    ...
    {
        await DeferAsync(ephemeral);
        var details = await GetDetailsAsync(user, note, context, category, ephemeral);
        ...
        [RequireEphemeralScope] bool ephemeral = false)
    {
        ...
        var details = await GetDetailsAsync(user, template.Reason, context, template.Category, ephemeral);
    ...
    public async Task BanAsync([RequireHigherRole] IUser user,
        ...
        BanModal modal)
        => await BanAsync(
            user, modal.DeleteDays, modal.Length, modal.Reason,
            modal.Context, modal.Category, modal.Ephemeral);
    ...
    public async Task HardMuteAsync([RequireHigherRole] IGuildUser user,
        ...
        HardMuteModal modal)
        => await HardMuteAsync(
            user, modal.Length, modal.Reason,
            modal.Context, modal.Category, modal.Ephemeral);
    ...
    public async Task RespondModMenuAsync(IUser user, IUserMessage? context, LogReprimandType[] options)
    {
        var selected = options.FirstOrDefault();
        if (selected is LogReprimandType.None) return;

        await RespondModMenuAsync(user, selected, context);
    }
    ...
    public async Task WarnAsync([RequireHigherRole] IGuildUser user,
        ...
        WarningModal modal)
        => await WarnAsync(
            user, modal.Amount, modal.Reason,
            modal.Context, modal.Category, modal.Ephemeral);

    private Task ReprimandWithModalAsync<T>(IUser user, IMessage? context, string? id) where T : ReprimandModal
        => Context.Interaction.RespondWithModalAsync<T>(id, modifyModal: m => m
            .WithTitle($"{m.Title} {user}")
            .UpdateTextInput(nameof(ReprimandModal.Context), b => b.WithValue(context?.GetJumpUrl())));

    private Task RespondModMenuAsync(IUser user, LogReprimandType type, IMessage? message = null) => type switch
    {
        LogReprimandType.Ban      => ReprimandWithModalAsync<BanModal>(user, message, $"ban:{user.Id}"),
        LogReprimandType.Kick     => ReprimandWithModalAsync<KickModal>(user, message, $"kick:{user.Id}"),
        LogReprimandType.Mute     => ReprimandWithModalAsync<MuteModal>(user, message, $"mute:{user.Id}"),
        LogReprimandType.Note     => ReprimandWithModalAsync<NoteModal>(user, message, $"note:{user.Id}"),
        LogReprimandType.Notice   => ReprimandWithModalAsync<NoticeModal>(user, message, $"notice:{user.Id}"),
        LogReprimandType.Warning  => ReprimandWithModalAsync<WarningModal>(user, message, $"warn:{user.Id}"),
        LogReprimandType.HardMute => ReprimandWithModalAsync<HardMuteModal>(user, message, $"hardMute:{user.Id}"),
        _ => throw new ArgumentOutOfRangeException(
            nameof(type), type, "Invalid Mod Menu option.")
    };

    private async Task<ReprimandDetails> GetDetailsAsync(
        IUser user, string? reason, IUserMessage? context, ModerationCategory? category, bool ephemeral)
    {
        var guild = await _db.Guilds.TrackGuildAsync(Context.Guild);
        var variables = guild.ModerationRules?.Variables;
        var details = new ReprimandDetails(
            Context, user, reason, variables, context,
    ...
    public abstract class ReprimandModal : IModal, ICategory, IEphemeral
    {
        ...
        [ModalTextInput(nameof(Context), TextInputStyle.Short, "https://discord.com/channels/...")]
        public IUserMessage? Context { get; set; }
        ...
        [RequiredInput(false)]
        [InputLabel("Category")]
        [ModalTextInput(nameof(Category), TextInputStyle.Short, "Default")]
        public ModerationCategory? Category { get; set; }

        [RequiredInput(false)]
        [InputLabel("Execute silently")]
        [ModalTextInput(nameof(Ephemeral), TextInputStyle.Short, initValue: "False")]
    ...
    public class WarningModal : ReprimandModal
    {
        ...
        [ModalTextInput(nameof(Amount), TextInputStyle.Short, initValue: "1")]
    ...
    public abstract class ExpirableReprimandModal : ReprimandModal
    {
        [RequiredInput(false)]
        [InputLabel("Length")]
        [ModalTextInput(nameof(Length), TextInputStyle.Short, "Example: 1h30m")]
    ...
    public class BanModal : ExpirableReprimandModal
    {
        ...
        [ModalTextInput(nameof(DeleteDays), TextInputStyle.Short, maxLength: 1, initValue: "0")]
